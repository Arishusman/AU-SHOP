...
async function getHomeReviews(){ try{ const res=await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/reviews`,{cache:"no-store"}); if(!res.ok)return []; const json=await res.json(); const items=Array.isArray(json?.data)?json.data:[]; return items.slice(0,5); }catch{return [];} }
async function getHomeConfig(){
  try{
    const res=await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/home-config`,{cache:'no-store'});
    if(!res.ok)return {categories:[],products:[]};
    const json=await res.json();
    return json?.ok && json?.data ? json.data : {categories:[],products:[]};
  }catch{
  ...
  try{
    ...
    return products.map(p=>{
      ...
      return live ? {
        ...p,
        ...live,
        slug: live.slug || p.slug || String(live.name || p.name).toLowerCase().normalize('NFKD').replace(/[^a-z0-9]+/g,'-').replace(/(^-|-$)/g,''),
        forWho: live.for_who ?? p.forWho
      } : p;
    });
  }catch{
    return products;
