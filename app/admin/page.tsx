'use client'; const API=process.env.NEXT_PUBLIC_API_URL||''; import {useMemo,useState,useEffect} from 'react';  import {categories} from '@/lib/categories'; import {money} from '@/lib/store'; import {Menu,X,Sun,Moon,Package,ShoppingCart,Star,FolderKanban} from 'lucide-react';
...
useEffect(()=>{
  ...
  const loadProducts=async()=>{
    try{
      ...
      if(!r.ok || !j.ok){
        throw new Error(j.error || 'Products could not be loaded');
      }

      setProd(Array.isArray(j.data?.items) ? j.data.items : []);
    }catch(e){
...
function HomePageCategories({categories}:{categories:any[]}){
  ...
  const saveOrder=async(next:any[])=>{
    ...
    const orderedItems=next.map((c:any,i:number)=>({
      category_id:c.id,
      sort_order:i
    }));
    ...
    try{
      ...
      const r=await fetch(API+'/api/categories/reorder',{
        method:'POST',
        headers:{
          'Content-Type':'application/json',
          Authorization:'Bearer '+(localStorage.getItem('au_admin_token')||'')
        },
        body:JSON.stringify({
          items:next.map((c:any)=>({id:c.id}))
        })
      });
      ...
      if(!r.ok||j.ok===false){
        throw new Error(j.error||'Unable to save category order');
      }
    }catch(e){
      console.error(e);
      alert(e instanceof Error?e.message:'Unable to save category order');
    }
  };
